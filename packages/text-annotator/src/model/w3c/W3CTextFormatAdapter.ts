...
export type W3CTextFormatAdapter<
  I extends TextAnnotation = TextAnnotation,
  E extends W3CTextAnnotation = W3CTextAnnotation
> = FormatAdapter<I, E>;
...
const parseW3CTextTargets = (annotation: W3CTextAnnotation) => {
  ...
  const parsed: TextAnnotationTarget = {
    ...
    styleClass:
      "styleClass" in w3cTargets[0] ? w3cTargets[0].styleClass : undefined,
  };
  ...
  for (const w3cTarget of w3cTargets) {
    ...
    const selector = w3cSelectors.reduce<Partial<TextSelector>>(
      (s, w3cSelector) => {
    ...
    if (isTextSelector(selector)) {
      parsed.outdated = w3cTarget.outdated;
      ...
      return {
        error: Error(
          `Missing selector types: ${missingTypes.join(
            " and "
...
>(
  ...
  const { selector, creator, created, updated, ...targetRest } = target;
